...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'distribute') {
    try {
        ...
        $sourceIds = array_map('intval', explode(',', $request['source_request_ids']));
        ...
        // Get consolidated items with approved quantities — explicit column list to avoid ambiguity
        $consolidatedItems = rows(
            'SELECT ri.id, ri.request_id, ri.item_id, ri.requested_quantity,
                    ri.ietw_recommended_qty, ri.gsssr_approved_qty, ri.issued_quantity,
                    i.item_name, i.unit
        ...
        // Prepare allocation array from POST
        $allocation = [];
        $errors = [];
        ...
        // Issue stock to each department based on allocation
        foreach ($allocation as $sid => $itemsAlloc) {
            foreach ($itemsAlloc as $itemId => $qty) {
                ...
                // Record OUTWARD transaction (deducts central stock + writes stock book)
                ...
                // Add to department inventory — qualified column to avoid ambiguity
                $stmt = $pdo->prepare(
                    "INSERT INTO department_inventory (department_id, item_id, quantity)
                     VALUES (?, ?, ?) AS new
                     ON DUPLICATE KEY UPDATE department_inventory.quantity = department_inventory.quantity + new.quantity"
                );
        ...
        // Mark consolidated request as issued
        $pdo->prepare("UPDATE requests SET status='ISSUED', admin_issued_by=?, admin_issued_at=NOW() WHERE id=?")
            ->execute([current_user()['id'], $consolidatedId]);
...
// Fetch approved consolidated requests waiting for distribution
...
<?php else: ?>
  <?php foreach ($pendingDistribution as $req):
    ...
    $sourceIds = array_map('intval', explode(',', $req['source_request_ids']));
    ...
    // Explicit column list to avoid ambiguity
    $consolidatedItems = rows(
        'SELECT ri.id, ri.request_id, ri.item_id, ri.requested_quantity,
                ri.ietw_recommended_qty, ri.gsssr_approved_qty, ri.issued_quantity,
                i.item_name, i.unit
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <?php foreach ($consolidatedItems as $ci):
                ...
                // Get original requested quantities from each source request
                $requestedQuantities = [];
                foreach ($sourceRequests as $src) {
                    $row = one('SELECT requested_quantity FROM request_items WHERE request_id=? AND item_id=?', [$src['id'], $ci['item_id']]);
                    $requestedQuantities[] = $row ? (int)$row['requested_quantity'] : 0;
                }
              ...
              <tr>
                ...
                <td class="text-center">
                  <input type="number" class="form-control form-control-sm text-center"
                         name="qty[<?= $ci['item_id'] ?>][<?= $src['id'] ?>]"
                         min="0" step="1" value="<?= e($allocations[$idx] ?? 0) ?>"
                         style="width: 100px; margin: 0 auto;" required>
